...
from spectrum_fundamentals.fragments import initialize_peaks, initialize_peaks_cms2
...
def generate_annotation_matrix_cms2(
    ...
    intensity[peak_pos_cms2(unmod_seq, crosslinker_position)[0]] = 0.0
    mass[peak_pos_cms2(unmod_seq, crosslinker_position)[0]] = 0.0
...
def parallel_annotate_cms2(
    spectrum: np.ndarray, index_columns: dict
    ...
    if "MODIFIED_SEQUENCE_A" in index_columns:
        mod_seq_column = "MODIFIED_SEQUENCE_A"

    fragments_meta_data, tmt_n_term, unmod_sequence, calc_mass_s, calc_mass_l = initialize_peaks_cms2(
    ...
    if len(matched_peaks) == 0:
        ...
        return intensity, mass, calc_mass_s, calc_mass_l, 0
    matched_peaks, removed_peaks = handle_multiple_matches(matched_peaks)
    intensities, mass = generate_annotation_matrix_cms2(
        matched_peaks, unmod_sequence, spectrum[index_columns["PRECURSOR_CHARGE"]]
    )
    return intensities, mass, calc_mass_s, calc_mass_l, removed_peaks

...
def annotate_spectra(un_annot_spectra: pd.DataFrame, mass_tolerance: Optional[float]) -> pd.DataFrame:
    """
    ...
    for row in un_annot_spectra.values:
        results = parallel_annotate(row, index_columns, mass_tolerance)
        if not results:
            continue
...
def parallel_annotate(
    spectrum: np.ndarray, index_columns: dict, mass_tolerance: Optional[float]

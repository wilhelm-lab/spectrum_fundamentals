        ...
        if self.xl:
            ...
            if self.true_intensities is not None and self.pred_intensities is not None:
                ...
                if self.sc_features_flag:
                    # b1 ions are thermodynamically unstable under HCD and rarely observed, so their
                    # absence penalises a correct PSM. The mask is derived from the ion layout rather
                    # than hard-coded: index 3:6 is b1 only for task="default".
                    self.metrics_val["spectral_angle_no_b1"] = SimilarityMetrics.spectral_angle(
                        self.true_intensities, self.pred_intensities, 0, masks=self.b1_mask()
                    )
